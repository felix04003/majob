...
export default function ClientTasksPage() {
  ...
  async function reload() {
    setLoading(true)
    setError(null)
    const res = await fetch(`/api/client/${eventId}/tasks`, {
      cache: "no-store",
    })
  ...
  async function loadComments(taskId: string) {
    ...
    const res = await fetch(
      `/api/client/${eventId}/tasks/${encodeURIComponent(taskId)}/comments`,
      { cache: "no-store" }
    )
    const json = await res.json().catch(() => ({}))
    if (res.ok) {
  ...
  async function addComment() {
    ...
    const res = await fetch(
      `/api/client/${eventId}/tasks/${encodeURIComponent(selectedTask.id)}/comments`,
      {
        method: "POST",
  ...
  async function validateTask(approved: boolean) {
    ...
    setValidating(true)
    const res = await fetch(
      `/api/client/${eventId}/tasks/${encodeURIComponent(selectedTask.id)}/validate`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
