import { NextResponse } from "next/server"
import { z } from "zod"
import { requirePlannerSession } from "@/lib/server/planner"
import { supabaseAdmin } from "@/lib/supabase/admin"
import { sendEmail } from "@/lib/email/send-email"
import { buildInvitationEmailHtml, buildInvitationEmailSubject } from "@/lib/email/invitation-email"

const Params = z.object({ token: z.string().min(10) })
const Body = z.object({
  channel: z.enum(["email", "whatsapp"]),
  baseUrl: z.string().url(),
})

export async function POST(req: Request, context: { params: Promise<{ token: string }> }) {
  const gate = await requirePlannerSession()
  if (!gate.ok) return gate.response

  const rawParams = await context.params
  const parsedParams = Params.safeParse(rawParams)
  if (!parsedParams.success) return NextResponse.json({ error: "Invalid token" }, { status: 400 })

  const json = await req.json().catch(() => null)
  const parsedBody = Body.safeParse(json)
  if (!parsedBody.success) return NextResponse.json({ error: "Invalid body", details: parsedBody.error.issues }, { status: 400 })

  const db = supabaseAdmin()
  const { channel, baseUrl } = parsedBody.data
  const token = parsedParams.data.token

  // Fetch invitation + guest + event
  const { data: inv, error: ie } = await db
    .from("invitations")
    .select("id, event_id, guest_id, invite_token, status, sent_at")
    .eq("invite_token", token)
    .single()

  if (ie || !inv) return NextResponse.json({ error: "Invitation not found" }, { status: 404 })

  const [{ data: guest }, { data: event }] = await Promise.all([
    db.from("guests").select("id, first_name, last_name, email, phone").eq("id", inv.guest_id).single(),
    db.from("events").select("id, title, start_at, venue_name, venue_address, invitation_template, invitation_custom").eq("id", inv.event_id).single(),
  ])

  if (!guest) return NextResponse.json({ error: "Guest not found" }, { status: 404 })
  if (!event) return NextResponse.json({ error: "Event not found" }, { status: 404 })

  const inviteUrl = `${baseUrl}/i/${inv.invite_token}`

  if (channel === "email") {
    if (!guest.email) {
      return NextResponse.json({ error: "Cet invité n'a pas d'adresse email" }, { status: 400 })
    }

    const html = buildInvitationEmailHtml({
      guestFirstName: guest.first_name,
      guestLastName: guest.last_name,
      eventTitle: event.title,
      eventDate: event.start_at,
      venueName: event.venue_name,
      venueAddress: event.venue_address,
      inviteUrl,
      templateId: event.invitation_template,
      customMessage: (event.invitation_custom as any)?.message || null,
    })

    const subject = buildInvitationEmailSubject(event.title)
    const result = await sendEmail({ to: guest.email, subject, html })

    if (!result.ok) {
      return NextResponse.json({ error: `Échec d'envoi: ${result.error}` }, { status: 500 })
    }

    // Mark as sent
    await db
      .from("invitations")
      .update({ sent_at: new Date().toISOString(), channel: "email", status: "sent" })
      .eq("id", inv.id)

    return NextResponse.json({ success: true, channel: "email" })
  }

  if (channel === "whatsapp") {
    if (!guest.phone) {
      return NextResponse.json({ error: "Cet invité n'a pas de numéro de téléphone" }, { status: 400 })
    }

    // Clean phone number (remove spaces, dashes, etc.)
    const cleanPhone = guest.phone.replace(/[\s\-()]/g, "")

    const title = event.title
    const date = new Date(event.start_at).toLocaleDateString("fr-FR", {
      weekday: "long",
      day: "numeric",
      month: "long",
      year: "numeric",
      hour: "2-digit",
      minute: "2-digit",
    })

    const message = [
      `✉️ *Invitation — ${title}*`,
      "",
      `Cher(ère) ${guest.first_name},`,
      "",
      `Vous êtes invité(e) à *${title}*`,
      `📅 ${date}`,
      event.venue_name ? `📍 ${event.venue_name}` : null,
      "",
      `Confirmez votre présence ici :`,
      inviteUrl,
    ]
      .filter(Boolean)
      .join("\n")

    const whatsappUrl = `https://wa.me/${cleanPhone}?text=${encodeURIComponent(message)}`

    // Mark as sent
    await db
      .from("invitations")
      .update({ sent_at: new Date().toISOString(), channel: "whatsapp", status: "sent" })
      .eq("id", inv.id)

    return NextResponse.json({ success: true, channel: "whatsapp", whatsappUrl })
  }

  return NextResponse.json({ error: "Invalid channel" }, { status: 400 })
}
