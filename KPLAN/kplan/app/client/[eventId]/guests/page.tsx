...
export default function ClientGuestsPage() {
  ...
  async function reloadChanges() {
    const c = await fetch(`/api/client/${eventId}/changes`).then((r) => r.json().catch(() => ({})))
    setChanges(Array.isArray((c as any)?.changes) ? (c as any).changes : [])
  }
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [g, c] = await Promise.all([
        fetch(`/api/client/${eventId}/guests`).then(async (r) => ({
          ok: r.ok,
          status: r.status,
          body: await r.json().catch(() => ({})),
        })),
        fetch(`/api/client/${eventId}/changes`).then(async (r) => ({
          ok: r.ok,
          status: r.status,
          body: await r.json().catch(() => ({})),
        })),
      ...
      if (cancelled) return
      if (!g.ok || !c.ok) {
        const gErr = !g.ok ? `guests: ${g.status} ${(g.body as any)?.error ?? ""}`.trim() : null
        const cErr = !c.ok ? `changes: ${c.status} ${(c.body as any)?.error ?? ""}`.trim() : null
      ...
      setGuests(Array.isArray((g.body as any)?.guests) ? (g.body as any).guests : [])
      setChanges(Array.isArray((c.body as any)?.changes) ? (c.body as any).changes : [])
      setLoading(false)
    }
    load()
  ...
  async function requestCreate() {
    ...
    const res = await fetch(`/api/client/${eventId}/guest-change`, {
      method: "POST",
  ...
  function handleFileSelect(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (ext !== "csv") {
      toast.error("Format non supporté. Veuillez exporter votre fichier Excel en .csv (UTF-8) puis réimporter.")
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (rows.length === 0) {
          toast.error("Aucun invité trouvé dans le fichier. Vérifiez que les colonnes Prénom et Nom sont présentes.")
          return
        }
  ...
  async function submitImport() {
    ...
    for (const row of importPreview) {
      ...
      if (row.plus_one_count) payload.plus_one_count = parseInt(row.plus_one_count) || 0
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Tabs defaultValue="official">
        <TabsList>
          <TabsTrigger value="official">Officiel ({guests.length})</TabsTrigger>
          <TabsTrigger value="pending">En attente ({changes.filter((c) => c.status === "pending").length})</TabsTrigger>
          <TabsTrigger value="history">Historique ({changes.length})</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="official">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Invités approuvés par le planner</CardDescription>
            </CardHeader>
            <CardContent>
              {loading ? (
                <div className="text-sm text-muted-foreground">Chargement…</div>
              ) : guests.length === 0 ? (
                <div className="text-sm text-muted-foreground">Aucun invité pour le moment.</div>
              ) : (
                    ...
                    <TableBody>
                      {guests.map((g) => (
                        <TableRow key={g.id}>
                          ...
                          <TableCell>{g.plus_one_count > 0 ? g.plus_one_count : "—"}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
        ...
        <TabsContent value="pending">
          <Card>
            <CardHeader>
              ...
              <CardDescription>En attente de validation par le planner</CardDescription>
            </CardHeader>
            <CardContent>
              {loading ? (
                <div className="text-sm text-muted-foreground">Chargement…</div>
          ...
          <Card>
            ...
            <CardContent>
              {loading ? (
                ...
                <ChangesList changes={changes} emptyMessage="Aucune demande soumise." />
              )}
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <DialogContent>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="mb-1 block text-sm font-medium">Accompagnants (+1)</label>
                <Input
                  type="number"
                  min={0}
                  max={10}
                  value={form.plus_one_count}
                  onChange={(e) => setForm({ ...form, plus_one_count: parseInt(e.target.value) || 0 })}
                />
              </div>
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium">Allergies / Régime</label>
              <Input
      ...
      <Dialog open={showImport} onOpenChange={setShowImport}>
        <DialogContent className="max-h-[80vh] max-w-3xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Importer {importPreview.length} invité(s)</DialogTitle>
          </DialogHeader>
          <div className="flex flex-col gap-4">
            <p className="text-sm text-muted-foreground">
              Vérifiez les données ci-dessous avant d'envoyer les demandes d'ajout. Chaque invité sera soumis à la validation du planner.
            </p>
            ...
            <div className="flex gap-2">
              <Button onClick={submitImport} disabled={importing}>
                {importing ? "Envoi en cours…" : `Envoyer ${importPreview.length} demande(s)`}
              </Button>
...
function ChangesList({ changes, emptyMessage }: { changes: Change[]; emptyMessage: string }) {
  if (changes.length === 0) {
      ...
      <Table>
        ...
        <TableBody>
          {changes.map((c) => {
            ...
            const contact = (p.email as string) || (p.phone as string) || "—"
            return (
              <TableRow key={c.id}>
                <TableCell className="font-medium">{name}</TableCell>
                <TableCell className="text-muted-foreground">{contact}</TableCell>
                <TableCell>
                  <Badge
                    variant={
                      c.status === "approved"
